...
config_phpstan() {
  ...
  if base=$(project_config phpstan.neon phpstan.neon.dist phpstan.dist.neon); then
    set_origin "project ($(basename "$base"))"
       ...
       && [[ -d "$QA_DRUPAL_ROOT/../composer/Generator" ]]; then
    # Core's PHPStan config analyses core *from a drupal/drupal checkout*: its
    # paths and ignoreErrors entries reference ../composer/, which only exists in
    # that layout. In a drupal/recommended-project install those paths are
    # missing and PHPStan refuses to start, so require the layout before using
    # it rather than failing with a wall of "is neither a directory" errors.
...
config_eslint() {
  local base stage overlay

  if base=$(project_config .eslintrc.json .eslintrc .eslintrc.js .eslintrc.yml eslint.config.js eslint.config.mjs); then
    set_origin "project ($(basename "$base"))"
  elif base=$(core_config core/.eslintrc.passing.json) && [[ -d "$QA_DRUPAL_ROOT/core/node_modules" ]]; then
    # Core's config only works when core's node_modules is installed: its
    # `extends` entries (airbnb-base, plugin:prettier/recommended) resolve
    # relative to the config file. Without a `yarn install` in core it would fail
    # with "Cannot find module", so fall through to the staged copy instead.
...
config_stylelint() {
  ...
  if c=$(core_config core/.stylelintrc.json) && [[ -d "$QA_DRUPAL_ROOT/core/node_modules" ]]; then
    set_origin "Drupal core"
    emit_config "$c"; return 0
  fi
